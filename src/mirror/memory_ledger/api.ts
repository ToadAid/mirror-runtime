...
import { initLedger, isLedgerEnabled } from "./db.js";

const LEDGER_ENABLED = isLedgerEnabled();

const _addMemoryEvent = LEDGER_ENABLED
  ? function addMemoryEvent(event: {
      kind: "memory" | "forget";
      key: string;
...
const _listMemoryEvents = LEDGER_ENABLED
  ? function listMemoryEvents(filters?: {
      user_id?: string;
      session_id?: string;
      kind?: "memory" | "forget";
      key?: string;
...
const _addMistakeEvent = LEDGER_ENABLED
  ? function addMistakeEvent(event: {
      category: string;
      summary: string;
      expected?: string;
      actual?: string;
  ...
  : function addMistakeEvent() {
      return { event_id: "", is_duplicate: false };
    };

const _listMistakeEvents = LEDGER_ENABLED
  ? function listMistakeEvents(filters?: {
      resolved?: number;
      tool_name?: string;
      category?: string;
      limit?: number;
...
const _resolveMistake = LEDGER_ENABLED
  ? function resolveMistake(id: string, notes?: string) {
      const database = getDb();
      database
...
export {
  _addMemoryEvent as addMemoryEvent,
  _listMemoryEvents as listMemoryEvents,
  _addMistakeEvent as addMistakeEvent,
  _listMistakeEvents as listMistakeEvents,
  _resolveMistake as resolveMistake,
};
