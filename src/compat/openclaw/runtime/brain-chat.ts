 ...
 *
 * Canonical Mirror chat execution lives in `src/mirror-runtime/`,
 * `src/mirror-provider/`, and `src/mirror-service/`.
 */
...
async function withMirrorCompatLogLevel<T>(fn: () => Promise<T>): Promise<T> {
...
export async function handleBrainChatEndpoint(
  ...
  const prepared = await withMirrorCompatLogLevel(async () =>
